            ...
            <>
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-gray-400">Espacio usado</span>
                  <span className="font-bold">{storage.usedMB} MB de {storage.limitMB} MB</span>
                </div>
                <div className="w-full bg-gray-800 rounded-full h-3">
                  <div
                    className={`h-3 rounded-full transition-all ${storage.alert ? 'bg-red-500' : storage.percentage > 60 ? 'bg-primary-400' : 'bg-green-500'}`}
                    style={{ width: `${Math.min(100, storage.percentage)}%` }}
                  />
                </div>
                <p className="text-right text-xs text-gray-400 mt-1">{storage.percentage}%</p>
              </div>
